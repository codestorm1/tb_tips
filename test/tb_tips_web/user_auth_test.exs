defmodule TbTipsWeb.UserAuthTest do
  use TbTipsWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias TbTips.Accounts
  alias TbTips.Accounts.Scope
  alias TbTipsWeb.UserAuth

  import TbTips.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TbTipsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TbTipsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TbTipsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
